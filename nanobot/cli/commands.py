...
app = typer.Typer(
    ...
    help=f"{__logo__} Foreman - quoting agent for small US machine shops",
...
def _print_agent_response(
    ...
    console.print()
    console.print(f"[cyan]{__logo__} Foreman[/cyan]")
    console.print(body)
    console.print()
...
async def _print_interactive_response(
    ...
    def _write() -> None:
        ...
        ansi = _render_interactive_ansi(
            lambda c: (
                c.print(),
                c.print(f"[cyan]{__logo__} Foreman[/cyan]"),
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} Foreman v{__version__}")
        raise typer.Exit()
...
def main(
    ...
    """Foreman - quoting agent for small US machine shops."""
    pass
...
def onboard(
    ...
    """Initialize Foreman configuration and workspace."""
    ...
    if wizard:
        ...
        try:
            ...
            console.print("[yellow]Please run 'foreman onboard' again to complete setup.[/yellow]")
            raise typer.Exit(1)
    ...
    agent_cmd = 'foreman agent -m "Hello!"'
    gateway_cmd = "foreman gateway"
    if config:
        agent_cmd += f" --config {config_path}"
        gateway_cmd += f" --config {config_path}"

    console.print(f"\n{__logo__} Foreman is ready!")
    console.print("\nNext steps:")
    if wizard:
...
def serve(
    ...
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show Foreman runtime logs"),
    workspace: str | None = typer.Option(None, "--workspace", "-w", help="Workspace directory"),
    config: str | None = typer.Option(None, "--config", "-c", help="Path to config file"),
):
    ...
    try:
        ...
        console.print("[red]aiohttp is required. Install with: pip install 'foreman-ai[api]'[/red]")
        raise typer.Exit(1)
...
def _run_gateway(
    ...
    *,
    ...
    console.print(f"{__logo__} Starting Foreman gateway version {__version__} on port {port}...")
...
def agent(
    ...
    logs: bool = typer.Option(False, "--logs/--no-logs", help="Show Foreman runtime logs during chat"),
):
...
def _get_bridge_dir() -> Path:
    ...
    if not source:
        ...
        console.print("Try reinstalling: pip install --force-reinstall foreman-ai")
        raise typer.Exit(1)
...
def status():
    ...
    console.print(f"{__logo__} Foreman Status\n")
